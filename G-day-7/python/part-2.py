...
def get_type(current_hand):
    counts = list(collections.Counter(current_hand).values())
...
@cache
def get_type_with_joker(current_hand):
    return min(
        get_type(current_hand.replace("J", replacement))
...
def compare_hands(hand_a, hand_b):
    type_a, type_b = get_type_with_joker(hand_a), get_type_with_joker(hand_b)
